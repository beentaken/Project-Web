...
void GameStatePlayUpdate(void)
{
	...
	if (0 == sgStopped || 1 == stopStep)
	{
		...
		{
			float t = ReflectAnimatedCircleOnStaticCircle(&pBall->posCurr, &newBallPos, BALL_RADIUS, &gPillarsCenters[i], gPillarsRadii[i], &intersectionPoint, &r);
	...
	for (i = 0; i < GAME_OBJ_INST_NUM_MAX; i++)
	{
		...
		Matrix2DScale		(&pInst->transform, pInst->scaleX,     pInst->scaleY);
		Matrix2DRotRad  		(&m,                pInst->dirCurr);
		Matrix2DConcat		(&pInst->transform, &m,               &pInst->transform);
		Matrix2DTranslate		(&m,                pInst->posCurr.x, pInst->posCurr.y);
		Matrix2DConcat       (&pInst->transform, &m, &pInst->transform);
	}
}
