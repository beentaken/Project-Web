...
class ChHashTable
{
  ...
  private:
    ...
    void remove_node(ChHTHeadNode *head, const char *Key);
    void DuplicateCheck(const ChHTHeadNode *head, const char *Key);
    void addtofront(ChHTHeadNode *head, ChHTNode *node);
    void GrowTable();

    ChHTHeadNode *table_;
    mutable HTStats stats_;
    const HTConfig *config_;
    bool CPPMemMngr_;
