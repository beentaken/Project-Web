...
class ObjectAllocator
{
  ...
  private:
    ...
    unsigned allocation_num_;
    GenericObject *PageList_;                      // the beginning of the list of pages
    GenericObject *FreeList_;                      // the beginning of the list of objects

    OAStats Stats_;                                // a pointer to the statistical info
    OAConfig Conf_;                                // a pointer to the configuration parameters
    ...
    bool is_on_freelist(void *Object) const;             // checks for the given object on the freelist
    bool not_on_boundry(void *Object);             // checks the address for boundry issues
    bool pad_corrupted(void *Object);              // corruption check
