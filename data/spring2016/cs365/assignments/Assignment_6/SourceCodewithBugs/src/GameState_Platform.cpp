		...
		for(j = 0; j < BINARY_MAP_HEIGHT; ++j)
		{
			switch(MapData[i][j])
			{
				...
				case TYPE_OBJECT_HERO:			//2
					Hero_Initial_X = i;
					Hero_Initial_Y = j;
					AEVec2Set(&Pos, Hero_Initial_X + 0.5f, Hero_Initial_Y + 0.5f);
...
void GameStatePlatformUpdate(void)
{
	...
	if((pHero->gridCollisionFlag & COLLISION_BOTTOM) && AEInputCheckCurr(DIK_SPACE))
		pHero->velCurr.y = JUMP_VELOCITY;

	if((pHero->gridCollisionFlag & COLLISION_RIGHT) && AEInputCheckCurr(DIK_SPACE))
	{
		pHero->velCurr.x = -JUMP_VELOCITY;
	}
	...
	for(i = 0; i < GAME_OBJ_INST_NUM_MAX; ++i)
	{
		...
		pInst->velCurr.y = GRAVITY*(gAEFrameTime + pInst->velCurr.y);
		...
		pInst->posCurr.y = pInst->velCurr.y * gAEFrameTime + pInst->posCurr.x;
	}
	...
	for(i = 0; i < GAME_OBJ_INST_NUM_MAX; ++i)
	{
		...
		//if(pInst->pObject->type == TYPE_OBJECT_HERO || pInst->pObject->type == TYPE_OBJECT_ENEMY1)
		{
			...
			if(pInst->gridCollisionFlag & COLLISION_RIGHT || pInst->gridCollisionFlag & COLLISION_LEFT)
			{
	...
	for(i = 0; i < GAME_OBJ_INST_NUM_MAX; ++i)
	{
		...
		if(0 == (pInst->flag & FLAG_ACTIVE) || 0 != (pInst->flag & FLAG_NON_COLLIDABLE))
			continue;


		switch(pInst->pObject->type)
		{
		//if pInst is an enemy
		case TYPE_OBJECT_ENEMY1:
			if(AETestRectToRect(&pHero->posCurr, pHero->scale, pHero->scale, &pInst->posCurr, pInst->scale, pInst->scale))
			{
				--HeroLives;
		...
		case TYPE_OBJECT_COIN:
			if(AETestCircleToRect(&pInst->posCurr, pInst->scale/2.0f, &pHero->posCurr, pHero->scale, pHero->scale))
			{
...
void GameStatePlatformFree(void)
{
	...
	for (u32 i = 0; i < GAME_OBJ_NUM_MAX; i++)
...
int ImportMapDataFromFile(char *FileName)
{
	...
	MapData = (int **)calloc(BINARY_MAP_WIDTH/2, sizeof(int *));
	for(i = 0; i < BINARY_MAP_WIDTH; ++i)
		...
		case STATE_GOING_LEFT:
			switch(pInst->innerState)
			{
				...
				case INNER_STATE_ON_UPDATE:
					...
					if((pInst->gridCollisionFlag & COLLISION_LEFT) || TYPE_OBJECT_EMPTY == GetCellValue(x, y))
					{
		...
		case STATE_GOING_RIGHT:
			switch(pInst->innerState)
			{
				...
				case INNER_STATE_ON_UPDATE:
					...
					//check if collided with left & checking checking if the left bottom cell is a non collision cell
					if((pInst->gridCollisionFlag & COLLISION_RIGHT) || TYPE_OBJECT_EMPTY == GetCellValue(x, y))
					{
						pInst->counter = ENEMY_IDLE_TIME;
						pInst->innerState = INNER_STATE_ON_EXIT;
						pInst->velCurr.x = 0.0f;
